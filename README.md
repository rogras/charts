...
There are the configs to start a permanent agents at the same time that you deploy the jenkins. With that you don't need to wait for the run of the first job.

| Parameter                  | Description                                     | Default                |
| -------------------------- | ----------------------------------------------- | ---------------------- |
| `permanentagent.podLabels` | Labels to create inside the pod of permanent agent  | `logging.elk.stack: platform`                |
| `permanentagent.resources`          | Resources allocation (Requests and Limits)      | `{requests: {cpu: 200m, memory: 256Mi}, limits: {cpu: 200m, memory: 256Mi}}`|
| `permanentagent.componentName`          | Jenkins Permanent Agent Name      | `jenkins-permanent-agent`|
| `permanentagent.swarmBin`          | Swarm bin file to execute at the container startup. More info at 'https://wiki.jenkins.io/display/JENKINS/Swarm+Plugin' | `nil`|
| `permanentagent.executors`          | Executors for each agent      | `nil`|
| `permanentagent.replicas`          | Replicas to deploy      | `nil`|
| `permanentagent.sspmutualtls.enabled`          | Enable the feature to add p12 certificates into the slave pods to connect with ssp | `false`|
| `permanentagent.sspmutualtls.ssldir`          | Directory to initialize the NSS db and copy de certs | `/etc/ssl/sspmutualtls`|
| `permanentagent.sspmutualtls.sspcrt`          | P12 certificate to import on the db | `nil`|
| `permanentagent.sspmutualtls.p12passwd`          | P12 password to import on the db| `nil`|
| `permanentagent.sspmutualtls.nsspasswd`          | Nss password to use the certificate with the curl comand | `nil`|
...
We added a flag to enable or disable proxy on all pods. 
This will add the environment variables (http_proxy/https_proxy) and config the update center to work with that

| Parameter                   | Description                                | Default                           |
| --------------------------- | ------------------------------------------ | --------------------------------- |
| `proxy.enabled`            | Enable the proxy connectivity         | `true`                           |
| `proxy.server`           | Proxy server                       | `proxyserv.svb.lacaixa.es`                       |
| `proxy.port`        | Proxy port                     | iam.amazonaws.com/role: `8080` |

## Build Images

With this feature you can enable the option to build images inside the agents.
This create the environment variables, volumes and give permissions to the agents.
The tool to build is podman.
NOTE: The part of 'securityContext' of the default variable 'agent.yamltemplate' needs to be deleted in order to run the agent with root user

| Parameter                   | Description                                | Default                           |
| --------------------------- | ------------------------------------------ | --------------------------------- |
| `buildImages.enabled`            | Enable the build images feature         | `false`                           |
| `buildImages.format`           | Format to force inside podman     | `nil`                       |
